...
async function main() {
  ...
  const timelock = await upgrades.deployProxy(
    SafeLandTimelock,
    [
      deployer.address,         // admin + canceller
      [deployer.address],       // proposers (deployer peut proposer)
      [deployer.address],       // executors (deployer peut exécuter)
    ],
    { initializer: "initialize" }
  );
  ...
  // ─── Timelock : rôles upgrade ────────────────────────────
  // NFT utilise UPGRADER_ROLE pour authorizeUpgrade
  await nft.grantRole(UPGRADER_ROLE, timelockAddress);
  console.log("   ✅ UPGRADER_ROLE → Timelock (NFT)");

  // Les autres contrats utilisent ADMIN_ROLE pour authorizeUpgrade
  await registry.grantRole(ADMIN_ROLE, timelockAddress);
  await escrow.grantRole(ADMIN_ROLE, timelockAddress);
  await fridda.grantRole(ADMIN_ROLE, timelockAddress);
  await justice.grantRole(ADMIN_ROLE, timelockAddress);
